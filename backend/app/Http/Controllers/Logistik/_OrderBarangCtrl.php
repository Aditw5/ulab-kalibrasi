...
class OrderBarangCtrl extends Controller
{
    ...
    public function batalKirimBarang(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $strukOrder = StrukOrder::where('norec', $request['norec_so'])->where('statusenabled', true)->where('kdprofile', $this->kdProfile)->first();

            if ($strukOrder['statusorder'] == 1) {
                $test = [];
                foreach ($request['details'] as $order) {
                    ...
                    $detailKirimProduk = OrderPelayanan::where('noorderfk', $request['norec_so'])->where('objectprodukfk', $order['objectprodukfk'])
                        ->where('kdprofile', $this->kdProfile)->where('statusenabled', true)
                        ->get();
                    // $detailKirimProduk = KirimProduk::where('nokirimfk', $strukKirim->norec)->where('objectprodukfk', $order['objectprodukfk'])->get();
                    // $jumlah = (float)$order['qtyprodukkonfirmasi'] * (float)$order['hasilkonversi'];
                    $saldoAkhirPengirim = (float)$saldoAwalPengirim + $order['qtyprodukkonfirmasi'];
                    ...
                    $this->kartu_STOK(array(
                        ...
                        "keterangan" => 'Batal Kirim Barang Order, dari Ruangan ' .  $request['namaruangantujuan'] . ' ke Ruangan ' .  $request['namaruanganasal'] . ' berupa produk ' . $order['namaproduk'] . '. No Order: ' .  $strukOrder->noorder,
                        "produkfk" => $order['objectprodukfk'],
                        "ruanganfk" =>  $request['ruangantujuanfk'],
                        "tglinput" => date('Y-m-d H:i:s'),
                        "tglkejadian" => date('Y-m-d H:i:s'),
                        "nostrukterimafk" => null,
                        "norectransaksi" => $strukOrder->norec,
            ...
            OrderPelayanan::where('noorderfk', $request['norec_so'])->where('statusenabled', true)->where('kdprofile', $this->kdProfile)
                ->update([
                    'qtyprodukkonfirmasi' => null,
                    'nostrukterimafk' => null,
                    'stokprodukdetailfk' => null
                ]);
