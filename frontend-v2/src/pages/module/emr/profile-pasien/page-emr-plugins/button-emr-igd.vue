<template>
  <div class="buttons">
    ...
    <VButton type="button" rounded outlined color="success" raised icon="lnir lnir-whatsapp"
      :disabled="NOREC_EMRPASIEN.length == 0 || (!props.isSelesai && props.registrasi?.objectdepartemenfk == 9)" @click="kirimWAResume()" v-if="!props.isHideCetak"> Kirim WA
    </VButton>
    <VButton type="button" rounded outlined color="warning" raised icon="lnir lnir-printer"
      :disabled="NOREC_EMRPASIEN.length == 0 || (!props.isSelesai && props.registrasi?.objectdepartemenfk == 9)" @click="print" v-if="!props.isHideCetak"> Cetak
    </VButton>
