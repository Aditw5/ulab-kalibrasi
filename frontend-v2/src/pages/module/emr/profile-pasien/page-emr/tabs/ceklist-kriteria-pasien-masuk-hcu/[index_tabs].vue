  ...
  <div class="columns is-12">
    <VCard>
      ...
      <table class="tbl-kmNICU">
        <thead>
          <tr>
            <th rowspan="2">Kriteria masuk HCU (Pilih sesuai dengan kondisi pasien)</th>
            <th colspan="2">Perlu perawatan HCU</th>
          </tr>
          <tr>
            <th>Ya</th>
            <th>Tujuan pelayanan HCU</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="kriteria in d_Edukasi">
            <td>{{kriteria.no}}. {{ kriteria.isi_kriteria }}</td>
            <td>
              <VField>
                <VControl raw subcontrol>
                  <VCheckbox v-model="input[kriteria.model]" color="primary" />
                </VControl>
              </VField>
            </td>
            <td>
              {{kriteria.tujuan_kriteria}}
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="tbl-kmNICU" style="margin-top: 30px;">
        ...
        <tr>
          <td>
            <div class="column is-12 text-center">Penilai: </div>
            <div class="column is-12">
              <VField>
                    <VControl class="prime-auto">
                      <AutoComplete v-model="input.dpjp" :suggestions="d_Pegawai" @complete="fetchPegawai($event)"
                        :optionLabel="'label'" :dropdown="true" :minLength="3" :appendTo="'body'"
                        :loadingIcon="'pi pi-spinner'" :field="'label'" placeholder="Nama Penilai" class="mt-2"/>
                    </VControl>
                  </VField>
            </div>
          </td>
...
import * as EMR from '../../../page-emr-plugins/ceklis-kriteria-pasien-masuk-hcu'
...
const d_Edukasi = ref(EMR.ceklisKriteriaPasienMasukHCU())
...
const COLLECTION: any = ref("ceklisKriteriaPasienMasukHCU") //table mongodb
