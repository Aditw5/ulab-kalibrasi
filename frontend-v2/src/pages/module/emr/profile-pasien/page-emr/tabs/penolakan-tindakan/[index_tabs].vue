      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <VField>
              <VControl>
                <VTextarea v-model="input.tindakanMedis" class="is-primary-focus" rows="2"
        ...
        <tr>
          ...
          <td>
            <VField>
              <VControl>
                <VTextarea v-model="input.risikoKomplikasi" class="is-primary-focus" rows="2" placeholder="Resiko dan Komplikasi...">
                </VTextarea>
              </VControl>
            </VField>
          </td>
...
const COLLECTION: any = ref("penolakanTindakan") //table mongodb
...
watch(
  ...
  (newValue, oldValue) => {
    input.value = {}
      ; (input.value.tanggalMRS = new Date()),  
        (input.value.tanggalMNICU = new Date()),  
        (input.value.tanggalKNICU = new Date()),  
        (input.value.tanggalVerif = new Date())
    setAutoFill()
    loadRiwayat()
    let rouutename = String(route.name) + '-' + route.params.index_tabs;
    let cache = H.cacheEMR().get(`TAB~${props.registrasi.noregistrasi}~${rouutename}`)
