                    ...
                    <div class="form-body">
                        <div class="columns is-multiline">
                            ...
                            <div class="column is-3 mt-0 " v-if="activeValue == 1">
                                <div class="columns is-multiline">
                                    <div class="column is-12 ">
                                      <VButton type="button" outlined color="primary" raised icon="feather:file"
                                        :loading="isLoading" @click="paketPreOp()"> Paket Pre Op
                                      </VButton>
                                      <VButton color="primary" circle icon="pi pi-ellipsis-v" raised
                                      @click="toggleOP($event, item)" class="ml-2"> Paket Lainnya
                                    </VButton>

                                        <div class="form-section  pt-0 pr-0 mt-3">
                                ...
                                <div class="form-section pt-0 pl-0">
                                    <div class="form-section-inner">
                                        ...
                                        <div class="columns is-multiline">
                                            ...
                                            <div class="columns is-multiline h-400-o">
          ...
          <div class="columns is-multiline">
            <div class="column is-6 pt-1 pb-1 ">
              <VButton type="button" icon="fas fa-print" class="w-100" circle outlined color="success" raised
                @click="paketLab('elektrolit')">
                Elektrolit
              </VButton>
            </div>
            <div class="column is-6 pt-1 pb-1 ">
              <VButton type="button" icon="fas fa-print" class="w-100" circle outlined color="success" raised
                @click="paketLab('fungsiginjal')">
                Fungsi Ginjal
              </VButton>
            </div>
            <div class="column is-6 pt-1 pb-1 ">
              <VButton type="button" icon="fas fa-print" class="w-100" circle outlined color="success" raised
                @click="paketLab('lablengkap')">
                Lab Lengkap
              </VButton>
            </div>
          </div>
  ...
  await useApi().get(
        `/laboratorium/list-dropdown`).then((response: any) => {
            ...
            if(props.registrasi.objectdepartemenfk == 9){
              ...
              if(props.registrasi.objectruanganfk == 222){
                item.ruanganTujuan = 199
                item.departemenfk = 3
              }
            }
...
const paketPreOp = async () => {
  const match = pasien.value.umur.match(/(\d+)thn/)
  const usia = match ? match[1] : null
  isLoading.value = true

  const idPaket = useApi().get(
        `/laboratorium/paket-pre-operasi?usia=${usia}`).then((response: any) => {
          const idOrder = response.split(',')
          idOrder.forEach(itemId=>{
            filteredLayanan.value.forEach(produk => {
              produk.details.forEach(detail => {
                if(detail.id == itemId){
                  item.produkCeklis[itemId] = true
                  listChecked.value.push({namaproduk: detail.namaproduk, hargasatuan: detail.hargasatuan, id: detail.id})
                }
              })
            })
          })
          H.alert('info', 'Paket pre Operasi terpilih')
          isLoading.value = false
        })
}

const paketLab = async (params) => {
  const idPaket = useApi().get(
        `/laboratorium/paket-laboratorium/${params}`).then((response: any) => {
          const idOrder = response.split(',')
          idOrder.forEach(itemId=>{
            filteredLayanan.value.forEach(produk => {
              produk.details.forEach(detail => {
...
const hasilItemsBrid = (e: any) => {
  if(e.ruanganlab == 'LAB. MIKROBIOLOGI'){
    H.alert('info', 'Untuk saat ini belum bisa cetak hasil di ruangan, silakan hubungi laboratorium')
      return
  }
    router.push({
