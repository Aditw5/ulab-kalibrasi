    ...
    <div class="columns is-12">
      <VCard>
        ...
        <div class="column is-12 p-0">
          <div class="is-flex">
            <div class="column is-2" style="margin-top:0.5rem">
              <span> Masuk PICU Tanggal :</span>
            </div>
            <div class="column is-10">
              <VField>
                <VControl class="prime-auto">
                  <Calendar v-model="input.tanggalMPICU" selectionMode="single" :manualInput="true" class="w-100"
                    :showIcon="true" showTime hourFormat="24" :date-format="'yy-mm-dd'" :disabled="item.tgl2" />
                </VControl>
              </VField>
            </div>
        ...
        <div class="column is-12 p-0">
          <div class="is-flex">
            <div class="column is-2" style="margin-top:0.5rem">
              <span> Keluar PICU Tanggal :</span>
            </div>
            <div class="column is-10">
              <VField>
                <VControl class="prime-auto">
                  <Calendar v-model="input.tanggalKPICU" selectionMode="single" :manualInput="true" class="w-100"
                    :showIcon="true" showTime hourFormat="24" :date-format="'yy-mm-dd'" :disabled="item.tgl2" />
                </VControl>
              </VField>
            </div>
        ...
        <table class="tbl-kmPICU">
          <thead>
            <tr>
              <th>KRITERIA MASUK PICU</th>
              <th>KRITERIA KELUAR PICU</th>
            </tr>
          </thead>
          <tbody>
            <td>
              <div class="column is-12" v-for="(data, index) in kriteriaMasuk">
                ...
                <VField :key="indexForm" v-for="(item, indexForm) in data.dataForm">
                  <VControl>
                    <VCheckbox v-model="input[item.model + item.no]" :label="item.value" class="p-0"
        ...
        <table class="tbl-kmPICU" style="margin-top: 30px;">
          <tr>
  ...
  import * as EMR from '../page-emr-plugins/checklist-kriteria-masuk-keluar-picu'
  ...
  const COLLECTION: any = ref("CheckListKriteriaMasukdanKriteriaKeluarRuangPICU") //table mongodb
  ...
  const simpan = () => {
    ...
    input.value.tanggalKPICU = H.formatDate(new Date(input.value.tanggalKPICU), 'YYYY-MM-DD HH:mm')
    input.value.tanggalMPICU = H.formatDate(new Date(input.value.tanggalMPICU), 'YYYY-MM-DD HH:mm')
