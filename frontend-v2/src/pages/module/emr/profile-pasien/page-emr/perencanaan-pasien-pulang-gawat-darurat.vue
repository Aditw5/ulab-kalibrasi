...
const riwayatOrderResep = async () => {
  await useApi().get(`/emr/get-riwayat-resep?nocmfk=${ID_PASIEN}&norec_pd=${props.registrasi.norec_pd}`).then((response: any) => {
    if (response.length <= 0) {
      ...
      let detailObatResep = '';
      response
        .filter((detail) => detail.satuanresep !== null)
        .map((detail, index, array) => {
          detailObatResep += `${detail.namaproduk} ${detail.aturanpakai} Jumlah(${detail.jumlah}) ${detail.satuanresep}`;
          if (index !== array.length - 1) {
            detailObatResep += ', ';
          }
        });
      input.value.obatObatanYangMasihDiminum = detailObatResep;
    }
  })
}
