  ...
  <div class="columns is-12">
    <VCard>
      ...
      <div class="column is-12 p-0">
        <div class="is-flex">
          <div class="column is-2" style="margin-top:0.5rem">
            <span> Masuk ICU Tanggal :</span>
          </div>
          <div class="column is-10">
            <VField>
              <VControl class="prime-auto">
                <Calendar v-model="input.tanggalMICU" selectionMode="single" :manualInput="true" class="w-100"
                  :showIcon="true" showTime hourFormat="24" :date-format="'yy-mm-dd'" :disabled="item.tgl2" />
              </VControl>
            </VField>
          </div>
      ...
      <div class="column is-12 p-0">
        <div class="is-flex">
          <div class="column is-2" style="margin-top:0.5rem">
            <span> Keluar ICU Tanggal :</span>
          </div>
          <div class="column is-10">
            <VField>
              <VControl class="prime-auto">
                <Calendar v-model="input.tanggalKICU" selectionMode="single" :manualInput="true" class="w-100"
                  :showIcon="true" showTime hourFormat="24" :date-format="'yy-mm-dd'" :disabled="item.tgl2" />
              </VControl>
            </VField>
          </div>
      ...
      <table class="tbl-kmICU">
        <thead>
          <tr>
            <th>KRITERIA MASUK ICU</th>
            <th>KRITERIA KELUAR ICU</th>
          </tr>
        </thead>
        <tbody>
          <td>
            <div class="column is-12" v-for="(data, index) in kriteriaMasuk">
              <div class="column is-12">{{ data.kategori }}</div>
              <VField :key="indexForm" v-for="(item, indexForm) in data.dataForm">
                <VControl>
                  <VCheckbox v-model="input[item.model + item.no]" :label="item.value" class="p-0" color="primary"
                    square />
                </VControl>
              </VField>
              <VField v-if="data.textArea">
                <VControl>
                  <VTextarea class="textarea" v-model="input[data.model + data.kategori]" rows="3"
          ...
          <td>
            ...
            <div class="column is-12" v-for="(data, index) in kriteriaKhusus">
              <VField :key="index">
                <VControl>
                  <VCheckbox v-model="input[data.model + data.no]" :label="data.value" class="p-0" color="primary"
                    square />
                </VControl>
              </VField>
            </div>
      ...
      <table class="tbl-kmICU" style="margin-top: 30px;">
        <tr>
          <td colspan="3" class="text-center">DOKTER PENANGGUNG JAWAB PASIEN R. IGD/ RANAP</td>
        </tr>
        <tr>
          <td>
            ...
            <div class="column is-12">
              <VField>
                <VControl class="prime-auto">
                  <Calendar v-model="input.tanggalVerifIGDRanap" selectionMode="single" :manualInput="true" class="w-100"
                    :showIcon="true" showTime hourFormat="24" :date-format="'yy-mm-dd'" :disabled="item.tgl2" />
                </VControl>
              </VField>
            </div>
          ...
          <td>
            ...
            <div class="column is-12">
              <VField>
                <VControl class="prime-auto">
                  <AutoComplete v-model="input.dpjpIGDRanap" :suggestions="d_Dokter" @complete="fetchDokter($event)"
      ...
      <table class="tbl-kmICU" style="margin-top: 30px;">
        <tr>
          <td colspan="3" class="text-center">DOKTER PENANGGUNG JAWAB PASIEN R. ICU</td>
        </tr>
        <tr>
          <td>
            <div class="column is-12 text-center">Tanggal & Jam: </div>
...
import * as EMR from '../page-emr-plugins/checklist-kriteria-masuk-keluar-icu'
...
const COLLECTION: any = ref("CheckListKriteriaMasukdanKriteriaKeluarRuangICU") //table mongodb
