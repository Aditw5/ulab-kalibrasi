...
:isLoading="isLoading" @simpan="simpan"
  ...
  <Fieldset v-for="(item, indexData) in input.details" :legend="`Peresetujuan/Penolakan ke ${item.no}`" :toggleable="true">
    ...
    <VCard>
      ...
      <div class="columns is-multiline">
        <div class="column is-3">
          <h1 class="mt-3">Nama Tindakan</h1>
        </div>
        <div class="column is-6">
          <VField class="pt-3">
            <VControl>
              <VInput type="text" v-model="item.namaTindakan" />
            </VControl>
          </VField>
        </div>
      </div>
      ...
      <div class="columns is-multiline">
        <div class="column is-4">
          <h1 class="mt-3">Diberikan pada tanggal / jam</h1>
        </div>
        <div class="column is-8">
          <VDatePicker class="pt-3" v-model="item.tglPemberian" color="green" trim-weeks mode="dateTime" :max-date="new Date()">
...
const COLLECTION: any = ref('informedConsentKemoterapi') //table mongodb
