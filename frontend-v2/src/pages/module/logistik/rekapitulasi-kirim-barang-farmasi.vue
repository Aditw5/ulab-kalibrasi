...
const cetakRekap = () => {
  ...
  H.printBlade(`report/farmasi/cetak-rekap-kirim-barang-farmasi${tglAwal}${tglAkhir}${ruanganfk}`)
}
...
const exportExcel = () => {
  const worksheet = XLSX.utils.aoa_to_sheet([
    ...
    [],
    ['REKAPITULASI KIRIM BARANG FARMASI'],
    [periode.value],
    [],
...
const saveAsExcelFile = (buffer: any, fileName: string) => {
  ...
  const desiredFileName = 'rekap_kirim_barang_farmasi' + EXCEL_EXTENSION;
...
async function fetchData() {
  ...
  await useApi().get('/logistik/rekap-kirim-barang-farmasi' + tglAwal + tglAkhir + '&offset=' + offset + '&limit=' + limit + '&rows=' + rows + produkfk + ruanganfk + ruanganfkTujuan).then((response) => {
