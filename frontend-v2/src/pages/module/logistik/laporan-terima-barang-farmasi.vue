<template>
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Laporan Terima Barang Farmasi {{ item.namaProdukHeader }}</h3>
    </div>
    <div class="coloumn">
...
const exportExcel = () => {
  remakeData.value = dataSource.value.map((e: any) => {
    return {
      ...
      'Ruangan Pengirim': e.ruangan_asal,
      'Ruangan Tujuan': e.ruangan_tujuan,
      'Penerima': e.namalengkap,
    }
  })
...
const saveAsExcelFile = (buffer: any, fileName: string) => {
  ...
  const desiredFileName = 'laporan_terima_barang_farmasi' + EXCEL_EXTENSION;
...
async function fetchData() {
  ...
  await useApi().get('/logistik/terima-barang-farmasi' + tglAwal + tglAkhir + '&offset=' + offset + '&limit=' + limit + '&rows=' + rows + produkfk + ruanganfk + ruanganfkTujuan).then((response) => {
