...
<template>
  ...
  <VCard>
    <div class="columns column">
      <h3 class="title is-5 mb-2 mr-1">Daftar Retur Barang Ruangan</h3>
    </div>
    <VButton color="info" RouterLink :to="{ name: 'module-logistik-retur-barang-ruangan' }" raised
      style="margin-left: 40rem;"><i class="fas fa-plus mr-3 p-0" aria-hidden="true"></i>Retur Barang</VButton>
              ...
              <div class="columns is-flex-tablet-p">
                <div class="column is-12">
                  <a class="flight-summary">
                    ...
                    <div class="meta" style="margin-left: 30px;">
                      <span style="text-align: center;">{{ items.jmlitem }}</span>
...
useHead({
  title: 'Daftar Retur Barang Ruangan - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchOrder = async () => {
  ...
  await useApi().get(
    `/logistik/daftar-retur-ruangan?offset=${offset}&limit=${limit}&rows=${rows}${dari}${sampai}${noorder}${produkfk}${ruanganasalfk}${ruangantujuanfk}`).then((response) => {
...
const editDis = (e: any) => {
  router.push({
    name: 'module-logistik-retur-barang-ruangan',
    query: {
      norec: e.norec,
      // isdistribusi: true,
    },
...
const cetakBuktiKirim = (e: any) => {
  H.printBlade(`logistik/cetak-bukti-terima-retur?nokirim=${e.nostruk}&jenis=retur`)
}
...
const saveBatal = async () => {
  ...
  await useApi().post('/logistik/batal-retur-barang-ruangan', param).then((response: any) => {
    isLoading.value = false
    fetchOrder()
    clearFilter()
  }, (error) => {
