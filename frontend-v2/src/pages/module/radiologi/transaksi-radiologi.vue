                          ...
                          <div style="max-height:500px;min-height: 300px; overflow-y: scroll;display: block;">
                            <tbody v-if="!isLoadingBill" v-for="(items, index)  in dataSourcefiltered" :key="index">
                              ...
                              <tr v-for="(itemsDet, index2)  in items.details" :key="index2">
                                ...
                                <td width="30%">
                                  <div class="columns is-multiline">
                                    <div class="column is-12">
                                      ...
                                      <div class="title-layan">{{ itemsDet.namaproduk }} {{ itemsDet.keteranganlainnya !== null && itemsDet.keteranganlainnya !== ''? `(${itemsDet.keteranganlainnya})`: '' }} - {{
                                ...
                                <td class="center" style="text-align:center">{{ itemsDet.jumlah }} {{ itemsDet.isExpertise }}</td>
...
const headerPasien = async (id: any, norec_pd: any, norec_apd: any, tglpelayanan: any) => {
  await useApi()
    .get(`/dashboard/headerpasien?nocmfk=${id}&norec_pd=${norec_pd}&norec_apd=${norec_apd}&tglpelayanan=${tglpelayanan}`)
    .then((response: any) => {
...
const fetchLayanan = async () => {
  ...
  dataSource.value = []
  await useApi().get(
    `/radiologi/layanan-radiologi?norec_pd=${NOREC_PD}&tglpelayanan=${TGLPELAYANAN}`).then(async (response: any) => {
...
const detail = (e: any) => {
  ...
  if (ID_PASIEN !== e.nocmfk || NOREC_PD !== e.norec_pd || NOREC_APD !== e.norec_apd || TGLPELAYANAN !== e.tglpelayanan) {
    ID_PASIEN = e.nocmfk;
    NOREC_PD = e.norec_pd;
    NOREC_APD = e.norec_apd;
    TGLPELAYANAN = e.tglpelayanan;
...
headerPasien(ID_PASIEN, NOREC_PD, NOREC_APD, TGLPELAYANAN)
listPegawai()
fetchPenunjang()
