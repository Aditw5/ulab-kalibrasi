                    ...
                    <div class="inner-box">
                      ...
                      <div class="card-balance-wrap">
                        ...
                        <div class="columns is-multiline mt-5">
                          ...
                          <div class="column is-12" v-if="item.changeNama">
                            <VField>
                              <VControl>
...
async function totalBayar() {
  ...
  await useApi().get(
    `/kasir/pembayaran-tagihan?norec=${item.NOREC_SP}&norec_pd=${item.NOREC_PD}`).then(async (response: any) => {
...
function simpan() {
  ...
  if (TITLE.value != 'Pembayaran Deposit' && TITLE.value != 'Pengembalian Deposit') {
    if (total < Math.round(item.jumlahBayar)) {
  ...
  var objSave = {
    ...
    jumlahbayar: parseFloat(item.jumlahBayar),
...
const cetakKwitansi = () => {
  ...
  qzService.printData('kasir/daftar-penerimaan/report/kwitansi?pdf=true&norec=' + norecSBM.value + '&namabaru=' + namabaru, 'KWITANSI', 1)
  // H.printBlade('kasir/daftar-penerimaan/report/kwitansi?norec=' + norecSBM.value + '&namabaru=' + item.namabaru);
}
