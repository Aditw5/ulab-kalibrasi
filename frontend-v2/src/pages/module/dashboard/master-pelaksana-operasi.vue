<template>
  <VCard>
      ...
      <div class="column is-12">
        ...
        <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
          <DataTable :value="dataSource" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
            ...
            <Column field="jenisoperasi" header="Jenis Operasi" :sortable="true" style="min-width:150px"></Column>
            <Column field="jenispetugaspe" header="Jenis Pelaksana" style="min-width:150px"></Column>
            <Column field="point" header="Point" :sortable="true" style="min-width:150px"></Column>
            ...
            <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
              <div class="tile-grid-item">
                <div class="tile-grid-item-inner">
                  ...
                  <div class="meta">
                    <span class="dark-inverted">{{ item.jenisoperasi }}</span>
                    <span>{{ item.jenispetugaspe}}</span>
    ...
    <VModal :open="modalInput" title="Tambah Master Pelaksana" actions="right" @close="modalInput = false">
      <template #content>
        <form class="modal-form">
          <div class="columns is-multiline">
            <div class="column is-12">
              <VField class="is-rounded-select is-autocomplete-select">
                <VLabel>Jenis Operasi</VLabel>
                <VControl icon="feather:search">
                  <Multiselect 
                    mode="single"
                    v-model="item.jenisoperasifk"
                    :options="d_ruangan"
            ...
            <div class="column is-12">
              <VField class="is-rounded-select is-autocomplete-select">
                <VLabel>Jenis Pelaksana</VLabel>
                <VControl icon="feather:search">
                  <Multiselect 
                    mode="single"
                    v-model="item.jenispelaksanafk"
                    :options="d_pegawai"
                    placeholder="Pilih data"
                    :searchable="true"
                    label="label"
            ...
            <div class="column is-12">
              <VField>
                ...
                <VControl icon="feather:bookmark">
                  <VInput type="textarea" v-model="item.point" placeholder="point" class="is-rounded" />
                </VControl>
              </VField>
            </div>
          </div>
...
await useApi().get('/dashboard/master-pelaksana-operasi?statusenabled=' + item.aktif ).then((response)=>{
...
function loadDropdown() {
  d_ruangan.value = []
  useApi().get(
    `/dashboard/master-pelaksana-operasi-dropdown`).then((response: any) => {
      d_ruangan.value = response.jenisoperasi.map((e: any) => { return { label: e.jenisoperasi, value: e.id } })
      d_pegawai.value = response.jenispelaksana.map((e: any) => { return { label: e.jenispetugaspe, value: e.id } })
    })
}
...
async function edit(e: any) {
  const response = await useApi().get(`/dashboard/master-pelaksana-operasi?id=${e.id_master}`);
  const datapelaksana = response;
  console.log(datapelaksana)

  item.value.id = e.id
  item.value.jenisoperasifk = e.jenisoperasifk
  item.value.jenispelaksanafk = e.jenispelaksanafk
  item.value.jenisoperasifk = d_ruangan.value.find((r: any) => String(r.value) === String(datapelaksana.id_jenisoperasi)) || null;
  item.value.jenispelaksanafk = d_pegawai.value.find((k: any) => String(k.value) === String(datapelaksana.id_jenispetugas)) || null;
  console.log(item.value.jenisoperasifk)
  console.log(item.value.jenispelaksanafk)
  item.value.point = e.point
  modalInput.value = true
}
...
async function save() {
  if (!item.value.jenisoperasifk) {
    useToaster().error('Jenis Operasi harus di isi')
    return
  }
  if (!item.value.jenispelaksanafk) {
    useToaster().error('Jenis Pelaksana harus di isi')
    return
  }
  if (!item.value.point) {
    useToaster().error('Point harus di isi')
    return
  }
  ...
  {
    'data': {
      'jenisoperasifk': item.value.jenisoperasifk,
      'jenispelaksanafk': item.value.jenispelaksanafk,
      'point': item.value.point,
    }
  }
  isLoadingTT.value = true
  await useApi().post(
    `/dashboard/save-master-pelaksana-operasi`, data).then((response: any) => {
