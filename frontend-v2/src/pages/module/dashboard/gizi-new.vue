                  ...
                  <div class="list-view list-view-v3">
                    <div class="search-menu-gizi" style="margin-bottom : 1rem;">
                      <div class="search-location-gizi" style="width: 100%">
                          ...
                          <input type="text" placeholder="Cari Nama Pasien, No Registrasi, No RM, BPJS, Atau NIK" v-model="item.search" v-on:keyup.enter="fetchPasien(order)" />
                      </div>
                      <VButton raised class="search-button-gizi" @click="fetchPasien()" :loading="isLoading"> Cari Data</VButton>
                    </div>
                    <VCard>
                      <VControl>
                        <Multiselect mode="single" v-model="item.filterRuanganRanap" :options="d_RuanganRanap" placeholder="Pilih ruangan" :searchable="true" autocomplete="off" @select="changeRuangRanap(item.filterRuanganRanap)" />
                      </VControl>
                ...
                <p v-else-if="activeValue === 'Riwayat'">
                  <div class="columns is-multiline">
                    ...
                    <div class="column is-3">
                      <VControl>
                        <Multiselect mode="single" v-model="item.filterRuanganOrder" :options="d_RuanganRanap" placeholder="Pilih ruangan" :searchable="true" autocomplete="off" @select="changeRuangOrder(item.filterRuanganOrder)" />
                      </VControl>
                    </div>
                    <div class="column is-4">
                      <VField>
                  ...
                  <div class="mb-3">
                    <VCard class="text-center pt-0 pb-0 mt-0">
                      <VRadio v-model="order" value="99" label="Semua" name="outlined_radio"
                          color="danger" />
                      <VRadio v-model="order" value="4" label="Pengiriman" name="outlined_radio"
                          color="warning" />
                  ...
                  <div class="list-view list-view-v3">
                    ...
                    <div v-else class="column is-12 p-0" style="overflow: scroll;height: 44rem;">
                      <div class="list-view-inner" style="max-height:500px;overflow: auto;">
                        <TransitionGroup name="list-complete" tag="div">
                          <Vcard>
                            <div v-if="dataRiwayat.length > 0" v-for="item in dataRiwayat" :key="item.id" class="list-view-item">
                              <div class="list-view-item-inner">
                                <VAvatar size="small" picture="/images/avatars/svg/pasien.svg" color="primary" bordered />
                                <div class="meta-left">
                                  <h3>
                                ...
                                <div class="meta-right">
                                  <div class="buttons">
                                      <VButton v-tooltip.bottom.left="'Detail'" label="Bottom Left" color="primary" circle icon="fas fa-info-circle" @click="showModalDetail(item)" :loading="isLoading"> Detail </VButton>
                                  </div>
                                </div>
                              </div>
                            </div>
...
async function fetchPasien() {
  ...
  let ruanganranapid = ''
  if (item.value.filterRuanganRanap) {
    ruanganranapid = item.value.filterRuanganRanap
  }
  let namapasien = ''
    , nocm = ''
    , noreg = ''
...
async function fetchRiwayat() {
  let ruanganid = ''
  if (item.value.filterRuanganOrder) { ruanganid = item.value.filterRuanganOrder }
  let qsearch = ''
...
function changeRuangRanap(e: any) {
