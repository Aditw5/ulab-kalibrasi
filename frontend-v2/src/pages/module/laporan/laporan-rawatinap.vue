...
<template>
  ...
  <div class="column">
    <VCard>
      <h3 class="title is-5 mb-2">Laporan Indexing Inap</h3>
      ...
      <div class="column" v-else>
        ...
        <div v-else>
          <DataTable :value="dataSource" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="20"
            ...
            <Column field="jenis_pasien" header="Cara Bayar" :sortable="true" style="min-width: 200px"></Column>
...
useHead({
  title: 'Laporan Indexing Pasien Rawat Inap - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
  ...
  await useApi().get(`pelayanan/get-laporan-ri?${tglAwal}${tglAkhir}${ruanganfk}${dokter}${nama}${kelompokpasien}${statuspasien}`).then((response: any) => {
...
const exportExcel = () => {
  remakeData.value = dataSource.value.map((e: any) => {
    return {
      ...
      KotaKabupaten: e.namakotakabupaten, Kecamatan: e.namakecamatan, Desa: e.namadesakelurahan, Provinsi: e.namapropinsi,
...
const saveAsExcelFile = (buffer: any, fileName: string) => {
  ...
  const desiredFileName = `laporan-rawat-inap${item.value.dokterfk ? '-'+item.value.dokterfk.label : ''}-${moment(item.value.qFilterTgl.start).format('YYYY-MM-DD')} sampai ${moment(item.value.qFilterTgl.end).format('YYYY-MM-DD')}${EXCEL_EXTENSION}`;
