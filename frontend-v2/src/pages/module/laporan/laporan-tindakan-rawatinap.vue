...
<template>
  ...
  <div class="column">
    <VCard>
      <h3 class="title is-5 mb-2">Laporan Indexing Rawat Inap</h3>
      ...
      <div class="column" v-else>
        ...
        <div v-else>
          <DataTable :value="dataSource" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="20"
            ...
            <Column field="kodetindakan" header="Kode Tindakan" :sortable="true" style="min-width: 200px"></Column>
            <Column field="namatindakan" header="Nama Tindakan" :sortable="true" style="min-width: 200px"></Column>
...
const fetchData = async () => {
  ...
  await useApi().get(`pelayanan/get-laporan-tindakan-ri?${tglAwal}${tglAkhir}${ruanganfk}${dokter}${nama}${kelompokpasien}${statuspasien}`).then((response: any) => {
...
const exportExcel = () => {
  ...
  remakeData.value = dataSource.value.map((e: any) => {
    return {
      ...
      KodeTindakan: e.kodetindakan, NamaTindakan: e.namatindakan, KotaKabupaten: e.namakotakabupaten, Kecamatan: e.namakecamatan, Desa: e.namadesakelurahan, Provinsi: e.namapropinsi,
