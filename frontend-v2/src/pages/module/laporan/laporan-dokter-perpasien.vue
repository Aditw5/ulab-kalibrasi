      ...
      <TabView class="tabview-custom " :scrollable="true" @tab-click="klikTab($event)">
            ...
            <div class="column" v-else>
              ...
              <div v-else>
                <DataTable :value="dataSource" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="10"
                  ...
                  <Column field="tglregistrasi" header="Tanggal Registrasi" frozen :sortable="true" style="min-width: 150px">
                  </Column>
                  <Column field="nocm" header="NOCM" :sortable="true" style="min-width: 100px"></Column>
                  ...
                  <Column field="noregistrasi" header="Noregistrasi" :sortable="true" style="min-width: 150px"></Column>
                  <Column field="kelompokpasien" header="Kelompok Pasien" :sortable="true" style="min-width: 100px"></Column>
                  <Column field="namakelas" header="Kelas" :sortable="true" style="min-width: 150px"></Column>
                </DataTable>
              </div>
            </div>
          </div>
        ...
        <TabPanel>
            ...
            <div class="search-widget">
              <div class="field">
                <div class="columns is-multiline">
                  ...
                  <div class="column is-3 mt-5">
                    <VIconButton type="button" color="success" class="searcv-button" raised icon="fas fa-search"
                      @click="fetchDataRanap()" :loading="isPlaceLoad">
                    </VIconButton>
                  </div>
                  <div style="margin-left: -21%" class="column is-4 mt-5 flex flex-wrap align-items-center justify-content-between gap-2">
                    <VButton color="warning" class="mr-4 mb-3" icon="fas fa-file-excel" raised @click="exportExcelRanap()">
                      Export
                      to
                      Excel </VButton>
                  </div>
          ...
          <div v-if="activeTab == 1">
            ...
            <div class="column" v-else>
              <VPlaceholderPage v-if="dataSourceRanap.length == 0" title="Data Tidak di Temukan."
                subtitle="Silakan filter pencarian di tanggal lain" larger>
                <template #image>
                  <img class="light-image" src="/@src/assets/illustrations/placeholders/search-1.svg" alt="" />
                  <img class="dark-image" src="/@src/assets/illustrations/placeholders/search-1-dark.svg" alt="" />
              ...
              <div v-else>
                <DataTable :value="dataSourceRanap" class="p-datatable-sm" :loading="isLoading" :paginator="true"
...
useHead({
  title: 'Laporan Dokter Perpasien - ' + import.meta.env.VITE_PROJECT,
})
...
const fetchData = async () => {
  ...
  await useApi().get(`/report/pelayanan/get-laporan-dokter-perpasien-rajal?${tglAwal}${tglAkhir}${dokter}`).then((response: any) => {
...
const fetchDataRanap = async () => {
  ...
  await useApi().get(`/report/pelayanan/get-laporan-dokter-perpasien-ranap?${tglAwal}${tglAkhir}${dokter}`).then((response: any) => {
    response.data.forEach((element: any, i: any) => {
      element.no = i + 1
    });
    dataSourceRanap.value = response.data
  })
  isPlaceLoad.value = false
...
const exportExcel = () => {
  const worksheet = XLSX.utils.aoa_to_sheet([
    ...
    [],
    ['LAPORAN DOKTER PERPASIEN RAWAT JALAN'],
    [periode.value],
    [],
    [
      'No', 'Tanggal Registrasi', 'NOCM', 'Nama Pasien', 'Ruangan Pasien', 'Dokter', 'Noregistrasi', 'Kelompok Pasien', 'Kelas'
    ],
    ...dataSource.value.map((e) => [
      e.no, e.tglregistrasi, e.nocm, maskNamaPasien(e.namapasien), e.namaruangan, e.dokter, e.noregistrasi, e.kelompokpasien, e.namakelas
    ]),
    [],
  ]);
...
const exportExcelRanap = () => {
  const worksheet = XLSX.utils.aoa_to_sheet([
    ['PEMERINTAH KOTA CIREBON'],
    ['RSD GUNUNG JATI'],
    ['Jl. Kesambi No.56, Kesambi, Kec. Kesambi, Kota Cirebon, Jawa Barat 45134 (0231) 206330'],
    [],
    ['LAPORAN DOKTER PERPASIEN RAWAT INAP'],
    [periode.value],
    [],
    [
      'No', 'Tanggal Registrasi', 'NOCM', 'Nama Pasien', 'Ruangan Pasien', 'Dokter', 'Noregistrasi', 'Kelompok Pasien', 'Kelas'
    ],
    ...dataSourceRanap.value.map((e) => [
      e.no, e.tglregistrasi, e.nocm, maskNamaPasien(e.namapasien), e.namaruangan, e.dokter, e.noregistrasi, e.kelompokpasien, e.namakelas
    ]),
    [],
  ]);
  ...
  const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
  saveAsExcelRanap(excelBuffer, 'products');
}
...
const saveAsExcelFile = (buffer: any, fileName: string) => {
  ...
  const desiredFileName = 'laporan_dokter_perpasien_rawat_jalan' + EXCEL_EXTENSION;
...
const saveAsExcelRanap = (buffer: any, fileName: string) => {
  let EXCEL_TYPE = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
  let EXCEL_EXTENSION = '.xlsx';
  const data: Blob = new Blob([buffer], {
    type: EXCEL_TYPE
