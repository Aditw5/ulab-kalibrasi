        ...
        <div class="search-widget">
          <div class="field">
            <div class="columns is-multiline">
              ...
              <div class="column is-4 pt-0 pb-0 is-4">
                <span>Dokter</span>
      ...
      <div class="column" v-else>
        ...
        <div v-else>
          <div class="column is-6">
            <DataTable :value="dataSourceKuota" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="10"
              ...
              <Column field="nama_ruangan" header="Nama Ruangan" frozen></Column>
              <Column field="jumlah_pasien" header="Jumlah Mobile JKN" frozen></Column>
            </DataTable>
          </div>
          <div class="my-5"></div>
          <DataTable :value="dataSource" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="10"
            ...
            <Column field="alamat" header="Alamat" :sortable="true" style="min-width: 200px"></Column>
            ...
            <Column field="tanggalreservasi" header="TGL Registrasi" :sortable="true" style="min-width: 140px"></Column>
          </DataTable>
        </div>
      </div>
    </VCard>
...
const fetchData = async () => {
  ...
  await useApi().get(`pelayanan/get-laporan-antrian-online?${tglAwal}${tglAkhir}&objectpegawaifk=${dokter}&q=${search}`).then((response: any) => {
    response.pasien.forEach((element: any, i: any) => {
      element.no = i + 1
    });
    response.kuota.forEach((element: any, i: any) => {
      element.no = i + 1
    });
    dataSource.value = response.pasien
    dataSourceKuota.value = response.kuota
  })
...
const exportExcel = () => {
  ...
  remakeData.value = dataSource.value.map((e: any) => {
    return {
      NamaPasien: maskNamaPasien(e.namapasien), NoRM: e.nocm, JenisKelamin: e.jeniskelamin,
