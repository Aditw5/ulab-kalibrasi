...
<template>
  ...
  <div class="column">
    <VCard>
      <h3 class="title is-5 mb-2">Laporan Indexing Rawat Jalan</h3>
      ...
      <div class="column" v-else>
        ...
        <div v-else>
          <DataTable :value="dataSource" class="p-datatable-sm" :loading="isLoading" :paginator="true" :rows="20"
            ...
            <Column field="namaruangan" header="Nama Poliklinik" :sortable="true" style="min-width: 200px"></Column>
...
const fetchData = async () => {
  ...
  await useApi().get(`pelayanan/get-laporan-indexing-rj?${tglAwal}${tglAkhir}${ruanganfk}${dokter}${nama}${kelompokpasien}${statuspasien}`).then((response: any) => {
...
const exportExcel = () => {
  ...
  remakeData.value = dataSource.value.map((e: any) => {
    return {
      NamaPasien: e.namapasien, NoRM: e.nocm, NoRegistrasi: e.noregistrasi, JenisKelamin: e.jeniskelamin, TanggalLahir: e.tgllahir, KelompokPasien: e.jenis_pasien,
      TanggalRegistrasi: e.tglregistrasi, Dokter: e.dokter, NamaPoli: e.namaruangan, Kelas: e.namakelas,
