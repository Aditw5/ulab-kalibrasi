...
const setujuiLaporan = async () => {
  ...
  isLoadingSave.value = true
  await useApi().post('/manager/save-setujui-laporan-repair', json).then((r) => {
    isLoadingSave.value = false
    toDashboard()
  }).catch((error: any) => {
    isLoadingSave.value = false
...
const cetakLaporanRepair = () => {
  H.printBlade(`manager/cetak-laporan-repair?pdf=true&norec=${dataSourceHasilLaporanRepair.value[0].norecregis}&norec_detail=${NOREC_DETAIL}`);
}
