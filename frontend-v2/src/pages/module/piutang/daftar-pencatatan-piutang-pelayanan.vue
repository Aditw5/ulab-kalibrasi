<template>
  ...
  <div class="column is-12">
    <VCard>
      <div class="column c-title pt-2 mb-3">
        <label class="title-page">Daftar Pencatatan Piutang Pasien</label>
      </div>
          ...
          <Column :exportable="false" header="Action" style="text-align: center;">
            <template #body="slotProps">
              <VIconButton type="button" icon="pi pi-pencil" class="mr-3" color="info" circle outlined raised
                v-tooltip.top="'Edit Rekanan'" :loading="slotProps.data.loading" @click="editRekanan(slotProps.data)">
              </VIconButton>
            </template>
          </Column>
        </DataTable>
...
const fetchData = async () => {
  ...
  await useApi().get(`piutang/daftar-piutang-layanan?${tglAwal}${tglAkhir}${kelompokpasien}${nama}${noregistrasi}${nocm}`).then((response: any) => {
