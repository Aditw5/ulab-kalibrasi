<template>
  ...
  <VCard>
    <span style="font-weight: bold; font-size: 13px; font-family: var(--font-alt);">
      ...
      <VTag color="warning" :label="item.namaruangan" rounded /> sejak Hari {{ H.formatDateIndo(item.tglmasuk) }}
    </span> <br>
    <span v-if="item.diagnosa" style="font-weight: bold; font-size: 13px; font-family: var(--font-alt);">
      Diagnosa Utama
      <VTag color="info" :label="item.diagnosa" rounded />, keterangan {{ item.ketdiagnosa }}
    </span>
    <span v-else style="font-weight: bold; font-size: 13px; font-family: var(--font-alt);">
      Diagnosa belum diinput oleh Dokter / Perawat
    </span>
                    ...
                    <table style="width: 100%; margin-top: 10px">
                      ...
                      <tr>
                        ...
                        <td class="font-values" width="60%">
                          {{ items.arrjenisdiet && items.arrjenisdiet !== '' ? items.arrjenisdiet : '-' }}
                        </td>
                      </tr>
                      <tr>
        ...
        <div class="columns is-multiline">
          ...
          <div class="column is-6">
            <VField class="is-autocomplete-select">
              <VLabel>Jenis Waktu</VLabel>
              <VControl icon="feather:list" class="prime-auto">
                <MultiSelect v-model="item.arrjeniswaktu" :options="d_JenisWaktu" filter optionLabel="label"
                  placeholder="Pilih Jenis Waktu" :maxSelectedLabels="3" style="width: 100%;" />
              </VControl>
            </VField>
          </div>
          <div class="column is-12">
          ...
          <div v-if="item.objectkategorydietfk == 23" class="column is-12">
            <VField class="is-autocomplete-select">
  ...
  if (props.pasien != undefined) {
    ...
    useApi()
      ...
      .then((response: any) => {
        ...
        if (response.diagnosa != null) {
          item.diagnosa = response.diagnosa.kddiagnosa + ' - ' + response.diagnosa.namadiagnosa
          item.ketdiagnosa = response.diagnosa.ketdiagnosis
        }
...
async function save(e: any) {
  ...
  // }
  if (!item.tglorder) {
    useToaster().warn('Tanggal Order harus di isi')
    return
  }
  if (!item.objectkategorydietfk) {
    useToaster().warn('Kategory Diet harus di isi')
    return
  }
  let datas: any = []
  if (item.objectkategorydietfk == 23) {
    if (!item.arrjenisdiet || item.arrjenisdiet.length == 0) {
      useToaster().warn('Kategori diet adalah cair, jenis diet harus di isi')
      return
    }
  ...
  if (!item.arrjeniswaktu || item.arrjeniswaktu.length == 0) {
    useToaster().warn('Jenis Waktu harus di isi')
    return
  }
  let details: any = []
  item.arrjeniswaktu.forEach(element => {
    details.push({
      'nocmfk': ID_PASIEN,
      'norec_pd': NOREC_PD,
      'objectjeniswaktufk': element.value,
...
function editItems(e: any) {
  if (e.statusorder != 0) {
    useToaster().warn('Status order sudah di verifikasi / sedang diproses tidak bisa diubah')
    return
  }
  ...
  item.arrjeniswaktu = [{
    label: e.jeniswaktu,
    value: e.objectjeniswaktufk,
    default: {
      id: e.objectjeniswaktufk,
...
function hapusItems(e: any) {
  ...
  useApi().post(
    `/dashboard/delete-order-gizi`, { norec: e.norec_op, norec_so: e.norec_so }).then((response: any) => {
      isLoading.value = false
      riwayatPasien()
    }).catch((e: any) => {
      isLoading.value = false
...
function clearInput() {
  ...
  delete item.arrjeniswaktu
  delete item.objectkategorydietfk
  delete item.arrjenisdiet
