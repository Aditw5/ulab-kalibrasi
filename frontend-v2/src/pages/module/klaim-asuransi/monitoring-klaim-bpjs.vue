        ...
        <div class="column is-12">
            ...
            <VCard v-else>
                <DataTable :value="dataSource" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]" scrollable :loading="isLoading"
                    ...
                    <Column field="kelompokpasien" header="Kelompok Pasien" style="min-width: 100px"></Column>
                    ...
                    <Column field="bed" header="Kamar dan Bed" style="min-width: 200px"></Column>
                    <Column field="dokter" header="Dokter" style="min-width: 200px"></Column>
                    <Column field="status" header="Status" style="min-width: 200px">
                        <template #body="slotProps">
                            <VTag :color="slotProps.data.status != null ? 'primary' : 'danger'" :label="slotProps.data.status != null ? slotProps.data.status : 'Belum Koding'" />
                        </template>
                    </Column>
                </DataTable>
            </VCard>
...
const fetchData = async () => {
    ...
    const response = await useApi().get(
        '/bridging/klaim/get-monitoring-klaim?tglawal='+ dari +'&tglakhir=' + sampai
        + '&search=' + search
        + '&ispulang=' + ispulang
        + '&ruanganid=' + ruanganid
        + '&depid=' + depid
...
const exportExcel = () => {
  ...
  remakeData.value = dataSource.value.map((e: any, index) => {
    return {
      No: e.no, NoRegistrasi: e.noregistrasi, TanggalPulang: e.tglpulang, NoRegistrasi: e.noregistrasi, NoRM: e.nocm, NamaPasien: e.namapasien, JenisKelamin: e.jeniskelamin, TanggalLahir: e.tgllahir, NoBPJS: e.nobpjs, NoSEP: e.nosep, TotalGrouping: e.inacbg_totalgrouper, Ruangan: e.namaruangan, TglMasuk: e.tglmasuk, kamarBed: e.bed, Dokter: e.dokter, Status: e.status ? e.status : 'belum koding'
    }
  })
