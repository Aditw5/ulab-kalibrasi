<template>
  ...
  <div class="columns column">
    <h3 class="title is-5 mb-2 mr-1">Kelompok Staf Medis</h3>
  </div>
    ...
    <div class="column is-8">
      ...
      <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
        <DataTable :value="dataSourcefiltered" class="p-datatable-sm" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
          ...
          <Column field="kelompokstafmedis" header="Jenis Kelamin" :sortable="true"></Column>
          ...
          <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
            <div class="tile-grid-item">
              <div class="tile-grid-item-inner">
                ...
                <div class="meta">
                  <span class="dark-inverted">{{ item.kelompokstafmedis }}</span>
        ...
        <VCard>
          <div class="columns is-multiline">
            ...
            <div class="column is-12">
              <VField label="Kelompok Staf Medis">
                <VControl icon="feather:bookmark">
                  <VInput type="text" v-model="item.kelompokstafmedis" placeholder="Kelompok Staf Medis" class="is-rounded" />
                </VControl>
              </VField>
            </div>
            <div class="column is-12">
...
async function fetchData() {
  ...
  let kelompokstafmedis = ''
  let ReportDisplay = ''
  let StatusEnabled = ''

  if (item.kelompokstafmedis) kelompokstafmedis = '&kelompokstafmedis' + item.kelompokstafmedis
  ...
  const response = await useApi().get(
    '/sysadmin/master-ksm?offset=' + offset +
    '&limit=' + limit +
    '&rows=' + rows +
    StatusEnabled + kelompokstafmedis + ReportDisplay
  )
  isLoading = false
...
function edit(e: any) {
  item.value.id = e.id
  item.value.kelompokstafmedis = e.kelompokstafmedis
...
async function save() {
  if (!item.value.kelompokstafmedis) {
    useToaster().error('KSM harus di isi')
    return
  }
  var objSave =
  {
    'kelompokstafmedis': {
      ...
      'kelompokstafmedis': item.value.kelompokstafmedis,
  ...
  await useApi().post(
    `/sysadmin/save-ksm`, objSave).then((response: any) => {
...
async function deleterow(e: any) {
  await useApi().post(
    `/sysadmin/delete-ksm`, { 'id': e.id }).then((response: any) => {
      fetchData ()
    }, (error) => {
    })
}
