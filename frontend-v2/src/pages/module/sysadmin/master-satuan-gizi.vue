<template>
    ...
    <VCard>
        <div class="columns column c-title">
            <h3 class="title is-5 mb-2 ml-2" style="z-index:1">Satuan Gizi</h3>
        </div>
                        ...
                        <div class="column is-12">
                            <VField label="Satuan Gizi">
                                <VControl icon="feather:bookmark">
                                    <VInput type="text" v-model="item.satuangizi" placeholder="Satuan Gizi"
                                        class="is-rounded" />
                                </VControl>
                            </VField>
                        </div>
            ...
            <div class="column is-8 mt-3">
                ...
                <div class="user-grid user-grid-v2" v-if="selectView == 'list'">
                    <DataTable :value="dataSourcefiltered" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25]"
                        ...
                        <Column field="satuangizi" header="Satuan Gizi" :sortable="true"></Column>
                        ...
                        <div v-for="(item, key) in dataSourcefiltered" :key="key" class="column is-6">
                            <div class="tile-grid-item">
                                <div class="tile-grid-item-inner" @click="edit(item)">
                                    ...
                                    <div class="meta">
                                        <span class="dark-inverted mb-2">{{ item.satuangizi }}</span>
...
const dataSourcefiltered = computed(() => {
    ...
    return dataSource.value.filter((items: any) => {
        return (
            items.satuangizi.match(new RegExp(filters.value, 'i'))
        )
    })
})
...
const fetchData = async (e: any) => {
    ...
    await useApi().get(`/sysadmin/master-satuan-gizi?${statusEnabled}`)
    .then((response: any) => {
...
const save = async (item: any) => {
    if (!item.satuangizi) {
        useToaster().warning('Satuan gizi harus di isi')
        return
    }
    ...
    await useApi().post(
        `/sysadmin/save-satuan-gizi`, objSave).then((response: any) => {
...
const deleterow = async (e: any) => {
    isLoading.value = true
    await useApi().post(
        `/sysadmin/delete-satuan-gizi`, { 'id': e.id }).then((response: any) => {
            isLoading.value = false
            clear()
            fetchData(true)
        }, (error) => {
...
const edit = (e: any) => {
    item.value.id = e.id
    item.value.satuangizi = e.satuangizi
