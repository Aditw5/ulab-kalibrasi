                            ...
                            <div class="right">
                                <div class="buttons">
                                    <VButton icon="lnir lnir-arrow-left rem-100"
                                        :to="{ name: 'module-sysadmin-master-map-paket-to-produk' }" light
                                        dark-outlined>
                                        Batal
                                    </VButton>
                                    <VButton type="button" icon="feather:save" :loading="isLoading" color="primary"
                                        raised @click="saveMapPaketToProduk()"> Simpan
                                    </VButton>
                                ...
                                <div class="column is-8">
                                    <VField class="cis-rounded-select_Z  is-autocomplete-select" v-slot="{ id }">
                                        ...
                                        <VControl icon="feather:search">
                                            <Multiselect mode="single" v-model="item.namapaket" :options="d_Paket"
                                                placeholder="Pilih data" :searchable="true" :attrs="{ id }"
                                                @select="handleChangeProduk(item.namapaket)" />
...
async function listDropdown() {
    const response = await useApi().get(
        `/sysadmin/master-map-paket-to-produk-dropdown`)
...
async function saveMapPaketToProduk() {
    if (!item.value.namapaket) { H.alert('warning', 'Nama Paket harus di pilih'); return }
    ...
    await useApi().post(
        `/sysadmin/save-map-paket-to-produk`, json).then((response: any) => {
