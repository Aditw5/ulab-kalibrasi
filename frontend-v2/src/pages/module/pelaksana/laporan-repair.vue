        ...
        <VCard>
          <div class="column is-12">
            ...
            <DataTable :value="dataSourceHasilLaporanRepair" :paginator="true" :rows="10"
              ...
              currentPageReportTemplate="Showing {first} to {last} of {totalRecords}" :loading="isLoading">
              <template #header>
                <div class="column pt-0 pb-0" v-if="dataSourceHasilLaporanRepair.length > 0">
...
const hapusLaporan = async (e: any) => {
  ...
  try {
    await useApi().post(`pelaksana/hapus-laporan-repair`, { norec: norec });
    clear()
    fetchData()
...
const hapusStatus = async (e: any) => {
  ...
  isLoadingSave.value = true;
  try {
    await useApi().post(`pelaksana/hapus-status-repair`, { norec: norec });
    clear()
    fetchData()
    isLoadingSave.value = false;
  } catch (error) {
...
const fetchData = async () => {
  ...
  await useApi().get(`pelaksana/get-laporan-repair?norecdetail=${NOREC_DETAIL}`).then((response: any) => {
